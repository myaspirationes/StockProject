...
# price_list = []
connect = MySQLdb.connect(host='localhost', user='root', passwd='root', db='test', port=3306)
cur = connect.cursor()
...
def get_price_data(url):
    ...
    for i in range(2, len(price_list)):
        ...
        try:
            sql = "INSERT   INTO web_django.sustained_higher_price (name, ts_code, `change`, sustain_days, " \
                  "turnover_rate, industry,date, insert_time) VALUES(%s, %s, %s, %s, %s, %s,CURRENT_DATE ,CURRENT_TIMESTAMP);"
            param = (name, ts_code, change, sustain_days, turnover_rate, industry)
            cur.execute(sql, param)
            print("add  sustained_higher_price success")
            connect.commit()
        except MySQLdb.Error as e:
            print("MySQL errors:%s" % (e.args[0], e.args[1]))
            connect.rollback()
...
def get_volum_data(url):
    ...
    for tr in tbody_val:
        ...
        print(industry)
        try:
            sql = "INSERT  INTO sustained_higher_volum (name, ts_code, benchmark_volum_date, " \
                  "sustain_days, price_change, industry,date,insert_time) VALUES(%s, %s, %s, %s, %s, %s,CURRENT_DATE ,CURRENT_TIMESTAMP)"
...
if __name__ == '__main__':
    ...
    url4 = 'http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/4/'
    # ,'http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/3/'

    price_url = ['http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/1/',
                 'http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/2/'
                 # 'http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/3/',
                 # 'http://data.10jqka.com.cn/rank/lxsz/field/lxts/order/asc/page/4/'
                 ]

    volum_url = ['http://data.10jqka.com.cn/rank/cxfl/field/count/order/desc/page/1/',
                 'http://data.10jqka.com.cn/rank/cxfl/field/count/order/desc/page/2/'
                 # 'http://data.10jqka.com.cn/rank/cxfl/field/count/order/desc/page/3/',
                 # 'http://data.10jqka.com.cn/rank/cxfl/field/count/order/desc/page/4/'
                ]
